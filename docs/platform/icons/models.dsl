  ...
  icons = container "Icons" {
    ...
    clients -> icons_controller "Requests icons for cleartext urls from" {
      perspectives {
        ...
        Icons 1.2.2 Server can tamper with URL data, causing clients to reveal vault contents without user input \n\n\
        Icons 1.2.3 Tracking of user vault contents by ip correlation between identity and icons services \n\n\
        Icons 1.2.4 No SLA offered on Icons service, graceful degradation of features needed if it goes down \n\n\
        Icons 1.2.5 SSRF through crafted input resolving to a location the server has elevated privileges in\
        "
      }
    }
